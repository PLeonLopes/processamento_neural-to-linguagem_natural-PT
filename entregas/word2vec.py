...
# Função sigmoid, que recebe um vetor e aplica a função sigmóide a ele
def sigmoid(x):
    """
...
# Função para determinar o custo e o gradiente softmax
def softmax(x):
    """
    Computa a função softmax para cada linha do input x.
    É crucial que essa função seja otimizada porque ela será frequentemente utilizada neste programa.
        Argumentos:
...
def naiveSoftmaxLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset):
    ...
    numWords, embDim = outsideVectors.shape

    # Calcula o produto escalar entre o vetor da palavra central e os vetores externos
    scores = np.dot(outsideVectors, centerWordVec)

    # Calcula as probabilidades usando a função softmax
    expScores = np.exp(scores - np.max(scores))  # Numericamente estável
    probs = expScores / np.sum(expScores)

    # Calcula a perda (custo) usando a função de perda softmax
...
def negSamplingLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset, K=10):
    ...
    # Calcula a perda (custo) usando amostragem negativa
    sigmoid_outside = sigmoid(scores[outsideWordIdx])
    sigmoid_neg = sigmoid(-scores[negSampleWordIndices])
    ...
    for neg_idx in negSampleWordIndices:
        gradOutsideVecs[neg_idx] += sigmoid(scores[neg_idx]) * centerWordVec
...
def skipgram(
    ...
    Argumentos:
        ...
        word2Ind -> um objeto dict que mapeia palavras para seus índices na lista de vetores de palavras

        centerWordVectors -> matriz dos vetores da palavra central (como linhas) com shape (num palavras no vocabulário, comprimento do vetor da palavra) para todas as palavras do vocabulário ( V no enunciado pdf)

        outsideVectors -> matriz dos vetores externos (como linhas) com shape (num palavras no vocabulário, comprimento do vetor da palavra)
                            para todas as palavras do vocabulário (U no enunciado pdf) 

        word2vecLossAndGradient -> a função de custo e gradiente para um vetor de predição dado os vetores de palavra outsideWordIdx, poderia ser um dos dois funções de perda que você implementou acima.

    Retorna:
        loss -> o valor da função de custo para o modelo skipgrama de (J no enunciado pdf) gradCenterVecs - o gradiente em relação ao vetor da palavra central com shape (comprimento do vetor da palavra,) (dJ / dV no enunciado pdf)

        gradOutsideVectors -> o gradiente em relação a todos os vetores de palavras externos com shape (num palavras no vocabulário, comprimento do vetor da palavra) (dJ / dU  no enunciado pdf)
    """
    ...
    # Índice da palavra central
    centerWordIdx = word2Ind[currentCenterWord]

    # Para cada palavra externa no contexto
    for outsideWord in outsideWords:
        outsideWordIdx = word2Ind[outsideWord]

        # Calcula a perda e os gradientes usando a função word2vecLossAndGradient
        ...
        # Atualiza os valores de perda e gradientes
        loss += loss_
